...
usersService.getUsers = async () => {
  const users = await db.query('SELECT id, firstName, lastName, email, role FROM tunniplaanUsers WHERE deleted = 0');
  return users;
};
...
usersService.createUser = async (newUser) => {
  ...
  const result = await db.query('INSERT INTO tunniplaanUsers SET ?', [user]);
...
usersService.deleteUser = async (id) => {
  ...
  await db.query('UPDATE tunniplaanUsers SET deleted = 1 WHERE id = ?', [id]);
  return true;
};
...
usersService.updateUser = async (user) => {
  ...
  await db.query('UPDATE tunniplaanUsers SET ? WHERE id = ?', [userToUpdate, user.id]);
  return true;
};
